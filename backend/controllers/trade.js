...
export const getTrades = (req, res) => {
    let q = 'SELECT s.symbol, t.*, c.exit_price, c.totalQuantity FROM symbol AS s RIGHT JOIN trade AS t ON t.symbol_id = s.id LEFT JOIN (SELECT t.id, e.exit_price, b.totalQuantity FROM trade t INNER JOIN journals.exit e ON t.id = e.trade_id INNER JOIN (SELECT trade_id, MAX(id) maxID, SUM(journals.exit.exit_quantity) totalQuantity FROM journals.exit GROUP BY trade_id) b ON e.trade_id = b.trade_id AND e.id = b.maxID) c ON t.id = c.id WHERE t.journal_id = ?;'
