          ...
          <thead>
            <tr>
              {journalType !== null && journalType === "Future"
                ? headings.futureTrade.map((heading, index) => (
            ...
            {tradeData.map((trade, index) => (
              <tr key={trade.id}>
                ...
                <td>
                  {(
                    trade.quantity *
                    (trade.entry_price - trade.stoploss)
                  ).toFixed(2)}
                </td>
                <td>
                  {(
                    ((trade.quantity * (trade.entry_price - trade.stoploss)) /
                      trade.account_value) *
                    100
                  ).toFixed(2)}
                  %
                ...
                <td>
                  {trade.totalQuantity
                    ? (trade.quantity / trade.totalQuantity).toFixed(2)
                    : "N/A"}
                </td>
                ...
                <td>
                  {trade.totalProfit === 0
                    ? "N/A"
                    : ((trade.totalProfit / trade.capital) * 100).toFixed(2)}
                  %
                </td>
                <td>{trade.capital}</td>
                <td>
                ...
                <td>
                  {((trade.totalProfit / trade.account_value) * 100).toFixed(2)}
                  %
                </td>
                  ...
                  0 ? (
                    ...
                    0 ? (
                      <span className={classes.minus}>
                        {trade.totalProfit -
                          (trade.totalCharges + trade.charges)}
                      </span>
                    ) : (
